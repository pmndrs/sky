...
| 4.4 | **Stylized looks** (gradient + Ghibli, unified — _in progress, `feat/sky-looks`_)       | [S]                     | Supersedes the old 4.4 "gradient mode" and 4.5 "Ghibli LUTs" rows, which specified two separate systems (`mode: 'gradient'` and `preset: 'ghibli'` + `styleStrength`). Design discussion concluded they are one system: a **look** — a colour ramp over `viewZenithCosAngle` plus a sun-relative tint over `lightViewCosAngle` — applied in `SkyAtmosphereMesh._buildColorNode` after the LUT/raymarch branches merge, so cube → PMREM → IBL inherit it and the sun/moon/star discs are excluded by construction. No `mode` field: `style` is orthogonal to the existing atmosphere `preset`. `styleStrength` splits into two axes — `chroma` (swap hue, keep physical luminance; scale-invariant, so haze can share it) and `value` (override luminance too; needed for artificial moonlight). A pure gradient override is `chroma: 1, value: 1`; Ghibli is `0.7 / 0.25`. Naming is `look` rather than `style` to avoid the React `style` prop and the taken `setPreset`. See `research/stylized-ghibli-sky.md` for the palette research and the 6 open questions (all now answered — see slices below). |
| 4.5 | **Unreal-parity artistic knobs**                                                        | [S]                     | Unreal's SkyAtmosphere splits artist control into two tiers, both automatically coherent across sky/AP/IBL because they never leave the physical domain. **Tier 1** (feeds the LUT bake): `multiScatteringFactor` (an openly non-physical gain Unreal ships), mie absorption colour/scale, rayleigh colour, ground albedo, density distributions — into `setAtmosphere` with convenience scalars alongside `setTurbidity`. **Tier 2** (uniforms, no rebake): `setSkyLuminanceFactor(color)` per-channel tint, `setAerialPerspectiveDistanceScale(n)` (a W-coord scale at AP sample time — the cheap "more fog" knob, zero dirty flags), `setTransmittanceMinSunElevation(deg)` (the sunset-goes-black hack). Keep `luminanceScale` (the 40× ILLUMINANCE_IS_ONE normalization) separate from `skyLuminanceFactor` — different jobs. Unreal ships no tier-3 colour remap; that is 4.4, and it is the only tier where haze coherence must be built rather than inherited.                                                                                                                                    |
...
| 4.7 | **Night sky rework** (_`feat/night-sky`_)                                               | [S]                     | Stars were baked into the 256² cube and came out as blurry multi-pixel blobs. Now split by frequency: resolved stars are energy-normalised PSF sprites in the main scene (`SkyStars`, ~0.01 ms/frame), the Milky Way is a galactic-frame glow map sampled by the sky mesh and baked into the cube (zero per-frame cost). Both are oriented by local sidereal time from the same solar model as the sun (registered to <0.2°), dimmed by transmittance, planet-occluded, and faded in by local contrast against the sky (log-space ramp — a linear ramp read as a shutter, a global zenith fade read as the whole band dimming at once). Procedural hash-grid and HDRI star paths removed (breaking, pre-1.0). Bench: `examples/vanilla/16-stars-bench.html` + `scripts/bench-stars.mjs`.                                                                                                                                                                                                                                                                                                                  |

#### 4.4 slice plan

Lives in [`SKY_LOOKS_PLAN.md`](SKY_LOOKS_PLAN.md) — the working plan for the
`feat/sky-looks` branch, including slice status and the answers to the research
memo's six open questions.
...
**2026-09-26 — night sky rework (4.7):**

- ✅ Stars out of the cube bake; `SkyStars` PSF sprites + Milky Way glow in
  the cube; `sky.enableStars()` rebuilt on top (`SkyNight`), demo 14 redone,
  HDRI example asset dropped.
- ✅ Measured (burst wall-clock, M5 Pro): live per-pixel background +
  procedural stars cost +1.4–1.7 ms at 4K vs ~+0.05 ms for sprites + baked
  Milky Way — the bake-first design stays.
- ⚠️ Found while measuring: a sun-change re-bake is ~9 ms and PMREM is ~95%
  of it (SkyView LUT 0.2 ms, cube faces 0.3 ms). That, not the background,
  is the real runtime cost of time-of-day animation — worth its own task.
