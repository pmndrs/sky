...
function equatorialDirection(raHours: number, decDegrees: number) {
  ...
  return new THREE.Vector3(cosDec * Math.cos(ra), Math.sin(dec), cosDec * Math.sin(ra))
}
...
const GALACTIC_BASIS_A = new THREE.Vector3().crossVectors(GALACTIC_NORTH, EQUATORIAL_NORTH).normalize()
const GALACTIC_BASIS_B = new THREE.Vector3().crossVectors(GALACTIC_NORTH, GALACTIC_BASIS_A).normalize()
 ...
 */
function celestialOrientation(latitude: number, dayOfYear: number, timeOfDay: number, north: string) {
  const obliquity = 23.4393 * DEG
  const solarLongitude = ((dayOfYear - 80.25) / 365.2422) * Math.PI * 2
  const solarRa = Math.atan2(Math.cos(obliquity) * Math.sin(solarLongitude), Math.cos(solarLongitude))
  const sidereal = solarRa + (timeOfDay - 12) * 15 * 1.0027379 * DEG
  ...
  const northRotation = north === '-Z' ? Math.PI : north === '+X' ? Math.PI / 2 : north === '-X' ? -Math.PI / 2 : 0

  return new THREE.Matrix4().makeRotationY(northRotation).multiply(horizon)
}
...
export function Stars({
  ...
  const { geometry, material, veilGeometry, veilMaterial } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (inMilkyWay) {
        ...
        const offset = (random() + random() + random() + random() - 2) * 0.12
        ...
        direction = new THREE.Vector3(Math.cos(ra) * radius, y, Math.sin(ra) * radius)
      }
      ...
      traits[i * 4] = Math.pow(random(), inMilkyWay ? 3 : 2.55) * (inMilkyWay ? 0.68 : 0.8)
    ...
    for (let i = 0; i < BRIGHT_STARS.length; i++) {
      ...
      traits[index * 4] = THREE.MathUtils.clamp(1 - (star.magnitude + 1.5) / 12, 0.46, 1)
    ...
    const emergence = TSL.smoothstep(threshold, threshold.add(0.34), uniforms.darkness)

    /** Fade stars near the horizon to keep the skyline clear. */
    const altitude = TSL.smoothstep(0.015, 0.1, TSL.positionWorld.normalize().y)
    ...
    const tint = TSL.mix(TSL.vec3(1.0, 0.94, 0.88), TSL.vec3(0.84, 0.9, 1.0), hue)
    ...
    const projColumns = TSL.cameraProjectionMatrix as unknown as ReturnType<typeof TSL.vec4>[]
    ...
    for (let i = 0; i < MILKY_WAY_CLOUDS; i++) {
      const angle = (i / MILKY_WAY_CLOUDS) * Math.PI * 2 + (random() - 0.5) * 0.16
    ...
    const veilMottle = TSL.sin(veilUv.x.mul(29).add(veilTrait.z.mul(3.1)))
      .mul(TSL.sin(veilUv.y.mul(23).sub(veilTrait.z.mul(1.7))))
      .mul(0.14)
      .add(0.86)
    const laneCenter = TSL.sin(veilUv.x.mul(8).add(veilTrait.z.mul(2))).mul(0.04)
    const dustLane = TSL.smoothstep(0.018, 0.1, TSL.abs(veilUv.y.sub(laneCenter)))
      .mul(0.42)
      .add(0.58)
    const veilAltitude = TSL.smoothstep(0.025, 0.14, TSL.positionWorld.normalize().y)
    ...
    const veilStrength = veilTrait.y
      ...
      .mul(uniforms.intensity)
      .mul(0.32)
    const veilTint = TSL.mix(TSL.vec3(0.23, 0.31, 0.56), TSL.vec3(0.42, 0.34, 0.38), veilTrait.w)
    ...
    const pixelScale = TSL.float(2).div(projColumns[1].y.mul(TSL.screenSize.y))
    veilMaterial.scaleNode = TSL.vec2(veilTrait.x, veilTrait.x.mul(TSL.float(0.55).add(veilTrait.w.mul(0.25)))).mul(
      pixelScale,
    )
    veilMaterial.colorNode = TSL.vec4(veilTint.mul(TSL.max(veilStrength, 0)), 1)
  ...
  return (
    <group matrix={orientation} matrixAutoUpdate={false}>
      <mesh geometry={veilGeometry} material={veilMaterial} frustumCulled={false} renderOrder={-2} />
