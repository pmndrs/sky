 ...
 */
export function fromUnitToSubUvs(u: any, resolution: any) {
  return u.add(float(0.5).div(resolution)).mul(resolution.div(resolution.add(float(1.0))))
}

export function fromSubUvsToUnit(u: any, resolution: any) {
 ...
 */
export const rayleighPhase = /*@__PURE__*/ Fn(([cosTheta]: any) => {
...
export const miePhaseCS = /*@__PURE__*/ Fn(([cosTheta, g]: any) => {
  const g2 = g.mul(g)
  const k = float(3.0).div(float(8.0).mul(PI)).mul(float(1.0).sub(g2)).div(float(2.0).add(g2))
 ...
 */
export const raySphereIntersectNearest = /*@__PURE__*/ Fn(([ro, rd, center, radius]: any) => {
  const a = dot(rd, rd)
  ...
  const candidate = select(
    ...
    select(
      onlySol0Neg,
      max(float(0.0), sol1),
      select(onlySol1Neg, max(float(0.0), sol0), max(float(0.0), min(sol0, sol1))),
    ),
  )
 ...
 * a full struct.
 */
export function computeScatteringAbsorption(height: any, params: any) {
 ...
 */
export function moveToTopAtmosphere(worldPos: any, worldDir: any, params: any) {
 ...
 */
export function uvToTransmittanceLutParams(uvNode: any, params: any) {
  const x_mu = uvNode.x
  const x_r = uvNode.y
 ...
 */
export function transmittanceLutParamsToUv(viewHeight: any, viewZenithCosAngle: any, params: any) {
  const topR2 = params.topRadius.mul(params.topRadius)
  const botR2 = params.bottomRadius.mul(params.bottomRadius)
  ...
  const discriminant = viewHeight.mul(viewHeight).mul(viewZenithCosAngle.mul(viewZenithCosAngle).sub(1.0)).add(topR2)
  const d = max(
    float(0.0),
    viewHeight
      .negate()
      .mul(viewZenithCosAngle)
      .add(sqrt(max(discriminant, float(0.0)))),
  )
 ...
 */
export const getSphericalDir = /*@__PURE__*/ Fn(([iPlusHalf, jPlusHalf, sqrtSampleCount]: any) => {
...
export function integrateScatteredLuminance({
  ...
  const extEps = extEpsNode !== undefined && extEpsNode !== null ? extEpsNode : float(1e-6)
