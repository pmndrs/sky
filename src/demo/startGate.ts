...
interface StartGateOptions {
  label?: string
  stages?: string[]
}

type MarkStage = (idx: number, status?: string) => void

interface StartGateRunContext {
  markStage: MarkStage
}

export function createStartGate({ label = 'Start', stages = [] }: StartGateOptions = {}) {
  ...
  btn.style.cssText =
    'padding:14px 28px;font-size:16px;border-radius:6px;border:0;cursor:pointer;background:#39c;color:#fff'

  const hint = document.createElement('div')
  hint.style.cssText = 'max-width:560px;font-size:13px;line-height:1.5;text-align:center;color:#bbb'
  hint.innerHTML =
    'Open the browser devtools console, then click <b>Start</b>.<br>' +
  ...
  const markStage: MarkStage = (idx, status = 'done') => {
  ...
  return {
    run(fn: (ctx: StartGateRunContext) => Promise<void> | void): Promise<void> {
 ...
 */
export async function logStage<T>(name: string, fn: () => Promise<T> | T): Promise<T> {
