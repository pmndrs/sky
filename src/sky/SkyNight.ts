import { HalfFloatType, LinearFilter, RepeatWrapping } from 'three/webgpu'
...
async function _loadEquirectHDR(url: string): Promise<any> {
  ...
  const Loader = isExr ? (loaderMod as any).EXRLoader : (loaderMod as any).RGBELoader
