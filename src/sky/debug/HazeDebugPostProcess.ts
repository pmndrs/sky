...
interface CreateHazeDebugOutputNodeArgs {
  scenePass: any
  ...
  raymarchJitterMode?: string
  raymarchExtEpsUniform?: any
  raymarchSegmentTUniform?: any
  raymarchTMaxScaleUniform?: any
...
export function createHazeDebugOutputNode({
  ...
  debugMode = null,
}: CreateHazeDebugOutputNodeArgs): any {
  ...
  return Fn(() => {
    ...
    if (enableRaymarchFallback) {
      If(useRaymarch, () => {
        ...
        const distForRm = raymarchTMaxScaleUniform ? distKmVar.mul(raymarchTMaxScaleUniform) : distKmVar
        ...
        const rmMeanTransmittance = rmTransmittance.x
          .add(rmTransmittance.y)
          .add(rmTransmittance.z)
          .mul(float(1.0 / 3.0))
        const rmAlpha = float(1.0).sub(rmMeanTransmittance).mul(validF)
        const rmMeanOpticalDepth = result.opticalDepth.x
          .add(result.opticalDepth.y)
          .add(result.opticalDepth.z)
          .mul(float(1.0 / 3.0))
