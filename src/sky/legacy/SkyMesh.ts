import { BackSide, BoxGeometry, Mesh, Vector3, NodeMaterial } from 'three/webgpu'
  ...
  constructor() {
    ...
    const colorNode = /*@__PURE__*/ Fn(() => {
      ...
      Lin.mulAssign(
        mix(
          vec3(1.0),
          pow(vSunE.mul(add(betaRTheta, betaMTheta).div(add(vBetaR, vBetaM))).mul(Fex), vec3(1.0 / 2.0)),
      ...
      const sundisc = smoothstep(sunAngularDiameterCos, sunAngularDiameterCos.add(0.00002), cosTheta).mul(
        this.showSunDisc,
      )
      ...
      const noise = Fn(([p_immutable]: any[]) => {
