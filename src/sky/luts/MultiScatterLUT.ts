...
export class MultiScatterLUT {
  ...
  _buildColorNode() {
    ...
    // Backend selection (WGSL_CORE_PLAN.md, seam 1). WGSL on WebGPU runs the
    // whole 64×20 nested integrator in the shared core; LUT sampling is manual
    // bilinear (~1e-3 vs hardware). Debug modes stay on the TSL path below.
    ...
    return Fn(() => {
      ...
      const sunDir = vec3(float(0.0), sqrt(saturate(float(1.0).sub(cosSunZenith.mul(cosSunZenith)))), cosSunZenith)
      ...
      Loop({ start: 0, end: SQRT_SAMPLE_COUNT * SQRT_SAMPLE_COUNT, type: 'int' }, ({ i: idx }: any) => {
        ...
        Loop({ start: 0, end: RAYMARCH_SAMPLE_COUNT, type: 'int' }, ({ i: s }: any) => {
