...
import { Fn, uv, vec3, vec4, float, sqrt, max, dot, normalize, uniform } from 'three/tsl'
...
export class SkyViewLUT {
  ...
  constructor(
    ...
    const initialSun = sunDirection instanceof Vector3 ? sunDirection.clone() : new Vector3(0.0, 0.0, 1.0)
  ...
  _buildColorNode() {
    ...
    // Backend selection (WGSL_CORE_PLAN.md, seam 1). On WebGPU the whole SkyView
    // pixel (integrator + LUT sampling) runs the shared WGSL core. LUT sampling
    // there is manual bilinear (three's wgslFn has no sampler), which matches the
    // TSL hardware-sampled path to ~1e-3 — visually identical, verified by
    // examples/parity/12-skyview-lut. WebGL keeps the TSL authoring below.
